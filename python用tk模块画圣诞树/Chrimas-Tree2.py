...
"""
speed("fast")  # 定义速度
screensize(100, 100, bg='black')  # 定义背景颜色，可以自己换颜色
left(90)
forward(3 * n)
color("orange", "yellow")  # 定义最上端星星的颜色，外圈是orange，内部是yellow
begin_fill()
left(126)
for star in range(5):  # 画五角星
    forward(n / 5)
    right(144)  # 五角星的角度
    forward(n / 5)
    left(72)  # 继续换角度
...
tree(15, n)
backward(n / 2)
for decoration in range(200):  # 循环画最底端的小装饰
    a = 200 - 400 * r.random()
    b = 10 - 20 * r.random()
    up()
    forward(b)
...
def draw_snow():  # 定义画雪花的方法
    ...
    for snow in range(200):  # 画多少雪花
        ...
        t.pd()  # 落笔，pd=pendown
        dens = 6  # 雪花瓣数设为6
        snow_size = r.randint(1, 10)  # 定义雪花大小
