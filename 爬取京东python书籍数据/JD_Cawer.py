...
class JD_Spider:
    ...
    def run(self):
        ...
        input_edit.clear()
        input_edit.send_keys(self.ite_name)
            ...
            try:
                product = {
                    ...
                    'price': item.find('div > div.p-price > strong > i').text().replace('\n', '\t') if item.find(
                        'div > div.p-price > strong > i').text() else "None",
                    'commit': item.find('div > div.p-commit > strong > a').text().replace('\n', '\t') if item.find(
                        'div > div.p-commit > strong > a').text() else "None",
                    'author': item.find('div > div.p-bookdetails > span.p-bi-name > a').text() if item.find(
                        'div > div.p-bookdetails > span.p-bi-name > a').text() else "None",
                    'store': item.find('div > div.p-shopnum > a').text().replace(' |', '').replace('\n',
                                                                                                   '\t') if item.find(
                        'div > div.p-shopnum > a').text() else "None",
                    'img': str(re.findall('.*?data-lazy-img="(//.*?.jpg)" source-data-lazy-img=.*?',
                                          str(item.find('div > div.p-img > a > img')))) or str(
                        re.findall('.*?"" src=(//.*?)" style="".*?',
                                   str(item.find('div > div.p-img > a > img')))) if item.find(
                        'div > div.p-img > a > img') else "None"
                }
                print(product)
                self.txt_file.write(','.join(
                    [product['name'], product['price'], product['commit'], product['author'], product['store'],
                     product['img']]))
                self.txt_file.write('\n')
            except Exception as e:
