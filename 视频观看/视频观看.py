...
class APP:
    def __init__(self, width=500, height=300):
        ...
        filemenu.add_command(label='使用说明', command=lambda: webbrowser.open(
            'http://blog.csdn.net'))
        filemenu.add_command(label='关于作者', command=lambda: webbrowser.open('https://github.com/luoshunpeng'))
    ...
    def video_play(self):
        ...
        if re.match(r'^https?:/{2}\w.+$', self.url.get()):
            if self.v.get() == 1:
                ...
                get_url_req = request.Request(url=get_url)
                ...
                # 请求播放,获取Success = 1
                get_movie_url = 'https://bd.jx.cn/?url='
                get_movie_data = {
                    'key': '%s' % now_key,
                    'time': '%s' % now_time,
                    'type': '',
                ...
                get_movie_req = request.Request(url=get_movie_url)
    ...
    def download_wmxz(self):
        if re.match(r'^https?:/{2}\w.+$', self.url.get()):
            ...
            # 获取保存视频的url
            get_url = 'https://www.1717yun.com/api/?url=%s' % ip
            get_url_req = request.Request(url=get_url)
            get_url_response = request.urlopen(get_url_req)
            get_url_html = get_url_response.read().decode('utf-8')
            bf = BeautifulSoup(get_url_html, 'lxml')
            ...
            # 获取视频地址
            get_movie_url = 'https://www.1717yun.com/api/?url='
            get_movie_data = {
                'up': '0',
                'url': '%s' % url,
            }
    ...
    def QR_Code(self):
        if re.match(r'^https?:/{2}\w.+$', self.url.get()):
            ...
            url = 'https://www.1717yun.com/api/?url=%s' % ip
            words = url
